...
let
  ...
  cffi-libffi = build-asdf-system {
    inherit (self.cffi-libffi) pname version asds lispLibs nativeLibs nativeBuildInputs;
    src = pkgs.fetchzip {
  ...
  cl-liballegro-nuklear = build-with-compile-into-pwd {
    inherit (self.cl-liballegro-nuklear) pname version src;
    nativeBuildInputs = [ pkgs.allegro5 ];
    nativeLibs = [ pkgs.allegro5 ];
    lispLibs = self.cl-liballegro-nuklear.lispLibs ++ [ self.cl-liballegro ];
  ...
  cl-fuse = build-with-compile-into-pwd {
    inherit (self.cl-fuse) pname version src lispLibs;
    nativeBuildInputs = [ pkgs.fuse ];
    nativeLibs = [ pkgs.fuse ];
  };

  cl-containers = build-asdf-system {
    inherit (self.cl-containers) pname version src;
    lispLibs = self.cl-containers.lispLibs ++ [ self.moptilities ];
    systems = [ "cl-containers" "cl-containers/with-moptilities" ];
  };

  swank = build-with-compile-into-pwd {
    inherit (self.swank) pname version src lispLibs;
  ...
  mathkit = build-asdf-system {
    inherit (self.mathkit) pname version src asds ;
    lispLibs = self.mathkit.lispLibs ++ [ self.sb-cga ];
  };

  nyxt-gtk = build-asdf-system {
    inherit (self.nyxt) pname;
    version = "2.2.4";

    lispLibs = self.nyxt.lispLibs ++ (with self; [
    ...
    installPhase = self.nyxt.installPhase + ''
      rm -v $out/nyxt
      mkdir -p $out/bin
      cp -v nyxt $out/bin
  ...
  ltk = self.ltk.overrideLispAttrs (o: {
    src = pkgs.fetchzip {
  ...
  magicl = build-with-compile-into-pwd {
    inherit (self.magicl) pname version src lispLibs;
